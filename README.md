...
- Sequential invoice draft numbers, locally persisted invoice states (draft/sent/paid), draft-PDF generation and validated final-PDF generation for sent/paid invoices, with Android share sheet.
...
- Group recorded expense-payment events by actual payment year and user-entered expense category in the tax overview; missing categories and payments whose expense record was deleted remain visible as separate groups. This is an organizational aid, not an ELSTER EÜR tax classification.
...
- Import local ISO 20022 CAMT.053 and MT940/Swift bank statements, deduplicate and store transactions, suggest invoice matches for incoming payments and expense matches for outgoing payments, and persist user-confirmed reconciliation links; marking an invoice paid always requires explicit confirmation. Imported transactions are included in backups and bookkeeping CSV exports.
- Show the last imported/authorized EUR account balances and local depot position values directly on the overview; mixed currencies, performance and tax calculations are intentionally not summed.
- Show a local bank-month view with income, expense, net balance and manual classification counts; it never changes bookkeeping or tax data.
- Add a persistent personal label to unmatched bank transactions (private, business income/expense, tax, transfer, or other); labels are included in encrypted backups and bookkeeping exports, but never alter tax totals or claim a tax classification.
- Import C24 and comdirect transaction CSV exports, C24 Excel (.xlsx) exports, C24 and comdirect Finanzreport PDFs, plus Trade Republic account-statement PDFs, native Trade Republic transaction CSV exports and pytr CSV exports locally on-device (including German localized headers, ISO timestamps, net amount/fee/tax handling and local per-ISIN position aggregation for native TR exports, in addition to CAMT.053); bank credentials are never requested or transmitted. The statement parsers accept only recognized layouts and reject ambiguous booking rows rather than guessing.
- Accept a shared C24/comdirect/Trade-Republic statement directly from Android's share sheet; KontoKlar opens the Banking page and imports the received content locally without uploading it.
...
- Delete unmatched imported bank transactions locally after confirmation; reconciled transactions are protected from deletion.
