...
Der Tag-Workflow testet die App, erstellt ein signiertes Release-APK, erzeugt zusätzlich eine `.sha256`-Prüfsummendatei und veröffentlicht beide als GitHub-Release-Assets. Tags müssen einmalig und aufsteigend sein; Android akzeptiert keine gleich alte oder niedrigere `versionCode`.
...
1. `versionCode` erhöhen und `versionName` setzen. Der aktuelle Quellstand ist `versionCode 93` / `versionName 0.84.0`; für die nächste Änderung beide Werte erhöhen.
