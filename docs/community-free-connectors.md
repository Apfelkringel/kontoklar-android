...
| Trade Republic privat | [pytr](https://github.com/pytr-org/pytr), [tr-api](https://github.com/cdamken/tr-api), [clitr](https://github.com/rtfpessoa/clitr) sowie der native Transaktionsexport | KontoKlar importiert sowohl den nativen TR-CSV-Export (`datetime,date,...,amount,fee,tax,...,transaction_id`) als auch pytr-CSV lokal. Beim nativen Export werden Betrag, Gebühr und Steuer zum Nettowert addiert; die Transaktions-ID verhindert Duplikate. Community-Abrufe mit privaten Endpunkten laufen weiterhin außerhalb der App. |
