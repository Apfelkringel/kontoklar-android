...
1. Nutze ausschließlich die Sandbox-Client-Credentials von finAPI. Vor dem ersten Deployment müssen `FINAPI_CLIENT_ID`, `FINAPI_CLIENT_SECRET` und `INSTALLATION_PEPPER` sicher auf dem Rechner bereitliegen: Cloudflare verlangt alle drei für den ersten Deploy. Verwende einen unabhängig erzeugten zufälligen Pepper mit mindestens 32 Zeichen und sichere ihn dauerhaft im Passwortmanager. Weder Credentials noch Pepper in Git, `.dev.vars`, Shell-History oder Chat ablegen. Bei Offenlegung müssen finAPI-Credentials rotiert werden. Den Pepper nicht spontan ersetzen: Provider-Passwörter werden davon abgeleitet, eine Rotation benötigt eine geplante Migration aller Provider-Identitäten.
...
4. Führe `npm run check` aus und deploye mit den drei Secrets über Wrangler `--secrets-file` aus einer geschützten lokalen Datei, die niemals ins Repository gelangt; alternativ kann ein erster, secret-freier Deploy nicht erfolgen, weil diese Secrets als erforderlich deklariert sind. Nach erfolgreichem Erstellen des Workers lassen sich einzelne Werte interaktiv über `npx wrangler secret put <NAME>` pflegen. Notiere die ausgegebene HTTPS-Worker-URL.
